...
import modelPath from "../../models/robot_dinosaur_toy.glb"
import "../Model.css"
import "../ShopItem.css"
...
const Scene = () => {
    ...
    return (
        <group ref={ref} scale={0.03} position={[0, 1, 0]}>
            <primitive object={gltf.scene} />
        </group>
    )
}

const DinoModel = ({ className }: DinoModelProps) => {
    ...
    return (
        ...
        >
            ...
            {visible && (
                <Canvas
                    className={`shop-model ${className ?? ''} dino`}
                    gl={{ antialias: false }}
                    id='shop-model'
                    shadows
                    onCreated={({ gl }) => {
