...
const Menu = ({ scrollTo }: MenuProps) => {
  ...
  return (
        <nav className='navigation' tabIndex={0} onBlur={() => setIsOpen(false)}>
            ...
            {/* Burger button — toggles between hamburger and close icon */}
            <button
            ...
            <ul className={`nav-menu ${isOpen ? 'nav-menu--open' : ''}`}>
                <li className='nav-link'>
                    <a 
                        ...
                        className='squircle-xl' 
                         onClick={(e: React.MouseEvent<HTMLAnchorElement>) => { e.preventDefault(); scrollTo('home'); }}
                    >
                        Home
                    </a>
                </li>
                <li className='nav-link'>
                    <a 
                        href='#about' 
                        className='squircle-xl' 
                        onClick={(e: React.MouseEvent<HTMLAnchorElement>) => { e.preventDefault(); scrollTo('about') }}
                    >
                        About
                        </a>
                </li>
                <li className='nav-link'>
                    <a 
                        href='#shop' 
                        className='squircle-xl' 
                    onClick={(e: React.MouseEvent<HTMLAnchorElement>) => { e.preventDefault(); scrollTo('shop') }}
                    >
                        Shop
                    </a>
                </li>
                <li className='nav-link'>
                    <a 
                        href='#contacts' 
                        className='squircle-xl' 
                        onClick={(e: React.MouseEvent<HTMLAnchorElement>) => { e.preventDefault(); scrollTo('contacts') }}
                    >
                        Contacts
                    </a>
                </li>
